...
async def judge_response(query, response, rubric):
    ...
    try:
        ...
        content = judge_res.content.replace("```json", "").replace("```", "").strip()
        start = content.find("{")
        end = content.rfind("}") + 1
...
async def run_senior_benchmark():
    ...
    for case in dataset:
            ...
            if "messages" in chunk:
                ...
                if hasattr(last_msg, "tool_calls") and last_msg.tool_calls:
                    for tc in last_msg.tool_calls:
                        ...
                        case_cost += cost
                        stats["tool_usage_cost"][t_name] = stats["tool_usage_cost"].get(t_name, 0.0) + cost
                ...
                if last_msg.type == "ai" and not last_msg.tool_calls:
                    ...
                    case_cost += TOOL_COSTS["default"] # Costo por respuesta final
        ...
        else: stats["failures"].append({"id": case["id"], "reason": evaluation["reason"]})

        stats["total_steps"] += num_steps
        stats["total_cost"] += case_cost

        print(f"CASE: {case['id']} | Score: {evaluation['score']}/5 | Cost: ${case_cost:.4f} | {'⭐' if is_success else '🔴'}")
